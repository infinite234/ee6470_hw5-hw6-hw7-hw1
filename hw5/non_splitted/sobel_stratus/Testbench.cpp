...
void Testbench::fetch_result() {
  ...
  sc_dt::sc_uint<24> total1;
  sc_dt::sc_uint<24> total2;
  sc_dt::sc_uint<24> total3;
#ifndef NATIVE_SYSTEMC
	i_resultr.reset();
 i_resultg.reset();
 i_resultb.reset();
  ...
  for (y = 0; y != height; ++y) {
    for (x = 0; x != width; ++x) {
#ifndef NATIVE_SYSTEMC
			total1 = i_resultr.get();
      total2 = i_resultg.get();
      total3 = i_resultb.get();
#else
			total1 = i_resultr.read();
      total2 = i_resultg.read();
      total3 = i_resultb.read();
#endif
			int result1 = (int)(total1);
      int result2 = (int)(total2);
      int result3 = (int)(total3);
      
      
      
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = result1;
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = result2;
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = result3;
