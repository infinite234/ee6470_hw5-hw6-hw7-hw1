...
for (y = 0; y != height; ++y) {
    for (x = 0; x != width; ++x) {
        ...
        // Convert into rgb
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = int(Red/total);
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = int(Green/total);
        *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = int(Blue/total);
      } 
